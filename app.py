...
# File paths
# ------------------------------------------------------------
# This app supports either GitHub layout:
#
# Option A, original package layout:
#   app.py
#   data/grandmas_course_points.csv
#   data/grandmas_course_markers.csv
#
# Option B, simple GitHub root layout:
#   app.py
#   grandmas_course_points.csv
#   grandmas_course_markers.csv
#
# Your current repository uses Option B, so this resolver checks both.
...
COURSE_POINTS_FILE = find_data_file("grandmas_course_points.csv")
COURSE_MARKERS_FILE = find_data_file("grandmas_course_markers.csv")
VIDEO_SYNC_FILE = find_data_file("video_mile_sync_template.csv")
SUMMARY_FILE = find_data_file("summary.json")
...
def load_markers(path: Path) -> pd.DataFrame:
    ...
    # Be forgiving about marker-file column names. Earlier prep files may have
    # used "type" or "category" instead of "marker_type".
    if "marker_type" not in df.columns:
        if "type" in df.columns:
            ...
            # Keep the app from crashing if a stripped-down marker file is used.
...
with st.sidebar:
    ...
    with st.expander("Resolved data files"):
        st.write(f"Course points: `{COURSE_POINTS_FILE}`")
        st.write(f"Markers: `{COURSE_MARKERS_FILE}`")
        st.write(f"Video sync: `{VIDEO_SYNC_FILE}`")
...
with st.expander("Video sync template"):
    if sync.empty:
        st.write(
            ...
            f"`{VIDEO_SYNC_FILE.name}` and add video seconds for known miles."
        )
        if VIDEO_SYNC_FILE.exists():
            st.dataframe(pd.read_csv(VIDEO_SYNC_FILE), use_container_width=True)
        else:
            st.info("No video sync template file found yet.")
    else:
