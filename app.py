...
def load_video_sync(path: str) -> pd.DataFrame:
    # Read as strings first to avoid pandas/pyarrow dtype issues with blank cells.
...
def render_youtube(video_second: int | float | None = None, autoplay: bool = True):
    """
    Reloads the embedded YouTube player at the selected timestamp.

    Streamlit cannot continuously control the YouTube player during the drag,
    but it reruns after the slider value changes. This function uses that rerun
    to rebuild the iframe with a new start timestamp.
    """
    start_arg = ""
    if video_second is not None:
        start_arg = f"&start={int(round(float(video_second)))}"

    # Most browsers block autoplay with sound, so mute=1 improves reliability.
    autoplay_arg = "&autoplay=1&mute=1" if autoplay else ""
...
with st.sidebar:
    ...
    if mode == "Video timestamp slider" and sync_available:
        ...
        jump_video = st.checkbox(
            "Auto-jump video when slider changes",
            value=True,
            help=(
                "When checked, moving the video timestamp slider reloads the "
                "YouTube embed at the selected timestamp. Streamlit updates after "
                "you release the slider."
            )
        )
        st.caption("Tip: the video jumps after you release the slider, not while dragging.")
...
with left:
    ...
    if jump_video and video_second is not None:
        render_youtube(video_second, autoplay=True)
    else:
        render_youtube(None, autoplay=False)

    if sync_available:
        st.info(
            "Move the Video timestamp slider in the sidebar. The video will jump "
            "to that timestamp after you release the slider, and the course data "
            "will update to the matching mile."
        )
    else:
        st.warning(
            "No usable video sync rows were found. Check that "
            "`video_mile_sync_template.csv` has columns like "
            "`video_second,mile` or `time_mmss,mile`."
        )
